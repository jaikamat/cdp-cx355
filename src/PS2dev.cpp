...
// Sony-optimized keystroke function based on 16.9344MHz vs 48MHz clock ratio
int PS2dev::keyboard_mkbrk_sony(unsigned char code)
{
#ifdef _PS2DBG
    _PS2DBG.print("keyboard_mkbrk_sony: Clock-ratio-adjusted make/break for code 0x");
    _PS2DBG.println(code, HEX);
#endif
    ...
    // Clock-ratio-based timing: 48MHz Arduino needs 2.84x longer delays
    // Sony's 16.9MHz expects ~100ms, so we need ~284ms
    delay(280);  // Clock-ratio adjusted key hold time
    keyboard_handle(&leds);
    ...
    // Clock-ratio-based inter-keystroke delay
    // Sony at 16.9MHz expects ~150ms, so we need ~426ms  
    delay(430);  // Clock-ratio adjusted inter-keystroke timing
    
#ifdef _PS2DBG
    _PS2DBG.print("keyboard_mkbrk_sony: SUCCESS for code 0x");
    _PS2DBG.println(code, HEX);
#endif
    return 0;
}
...
void PS2dev::sendKeySony(char c)
{
    if (c < 0 || c >= 128) return;
...
#ifdef _PS2DBG
    _PS2DBG.print("sendKeySony() called for: '");
    _PS2DBG.print(c);
    _PS2DBG.print("' -> scan code 0x");
    _PS2DBG.print(sc.code, HEX);
    if (sc.shift) _PS2DBG.print(" (with shift)");
...
#endif
        keyboard_press(0x12); // Left shift make
        delay(240); // Clock-ratio adjusted shift registration time (85ms * 2.84)
    }
    
#ifdef _PS2DBG
    _PS2DBG.println("  Sending character make/break with 1990s timing...");
#endif
    keyboard_mkbrk_sony(sc.code);  // Use 1990s-compliant timing
...
#endif
        keyboard_release(0x12); // Left shift break
        delay(240); // Clock-ratio adjusted shift clear time (85ms * 2.84)
    }
    
#ifdef _PS2DBG
    _PS2DBG.print("sendKeySony() completed for: '");
    _PS2DBG.print(c);
    _PS2DBG.println("'");
#endif
}

void PS2dev::sendStringSony(const String& str)
{
#ifdef _PS2DBG
    _PS2DBG.print("sendStringSony() called for: \"");
    _PS2DBG.print(str);
    _PS2DBG.println("\"");
#endif
    
    for (unsigned int i = 0; i < str.length(); i++) {
        sendKeySony(str.charAt(i));
        // No additional delay - keyboard_mkbrk_sony already includes proper timing
    }
    
#ifdef _PS2DBG
    _PS2DBG.println("sendStringSony() completed");
#endif
}

void PS2dev::sendEnterSony()
{
#ifdef _PS2DBG
    _PS2DBG.println("Sending Enter key with Sony timing...");
#endif
    keyboard_mkbrk_sony(0x5A);  // Use Sony-specific timing for Enter
#ifdef _PS2DBG
    _PS2DBG.println("Enter key sent with Sony timing");
#endif
}

void PS2dev::writeTitleToSony(const String& title)
{
#ifdef _PS2DBG
    _PS2DBG.print("writeTitleToSony() starting for title: \"");
    _PS2DBG.print(title);
    _PS2DBG.println("\"");
#endif
    
    // Step 1: Enter text entry mode using Enter key (as confirmed working)
#ifdef _PS2DBG
    _PS2DBG.println("Step 1: Entering text entry mode...");
#endif
    sendEnterSony();
    delay(2270); // Clock-ratio adjusted: 800ms * 2.84 for mode switch
    
    // Step 2: Clear any existing text using Shift+Delete (Sony documentation method)
#ifdef _PS2DBG
    _PS2DBG.println("Step 2: Clearing existing text...");
#endif
    sendShiftDelete();
    delay(1420); // Clock-ratio adjusted: 500ms * 2.84 for clear operation
    
    // Step 3: Type the title with clock-ratio timing
#ifdef _PS2DBG
    _PS2DBG.println("Step 3: Typing title with clock-ratio timing...");
#endif
    sendStringSony(title);
    
    // Step 4: Clock-ratio pause before confirmation
#ifdef _PS2DBG
    _PS2DBG.println("Step 4: Clock-ratio wait for Sony to be ready...");
#endif
    delay(1420); // Clock-ratio adjusted: 500ms * 2.84 for settle time
    
    // Step 5: Confirm the title with Enter
#ifdef _PS2DBG
    _PS2DBG.println("Step 5: Confirming title...");
#endif
    sendEnterSony();
    
    // Step 6: Final clock-ratio delay for save operation
    delay(2270); // Clock-ratio adjusted: 800ms * 2.84 for save operation
    
#ifdef _PS2DBG
    _PS2DBG.print("writeTitleToSony() completed for: \"");
    _PS2DBG.print(title);
    _PS2DBG.println("\"");
#endif
}